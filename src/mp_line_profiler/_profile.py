...
def profile(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        output_file = _DIR / f"{func.__name__}_{os.getpid()}_{os.getppid()}.lprof"
...
def average(stats_objs: list[line_profiler.LineStats], f):
    if len(stats_objs) > 1:
...
def _max(stats_objs: list[line_profiler.LineStats], f):
    if len(stats_objs) > 1:
        # Add from small scaling factors to large to minimize
        # rounding errors
        stats_objs = sorted(stats_objs, key=operator.attrgetter('unit'))
